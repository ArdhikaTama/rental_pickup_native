...
// 1. PROSES TAMBAH JAMINAN
if (isset($_POST['simpan_jaminan'])) {
    $nama_jaminan = mysqli_real_escape_string($koneksi, $_POST['nama_jaminan']);
    $keterangan   = mysqli_real_escape_string($koneksi, $_POST['keterangan']);

    $insert = "INSERT INTO jenis_jaminan (nama_jaminan, keterangan) VALUES ('$nama_jaminan', '$keterangan')";
    if (mysqli_query($koneksi, $insert)) {
        $pesan = "<div class='alert alert-success small py-2'>Jenis jaminan berhasil ditambahkan!</div>";
    } else {
        $pesan = "<div class='alert alert-danger small py-2'>Gagal menambahkan data: " . mysqli_error($koneksi) . "</div>";
    }
}

// 2. PROSES HAPUS JAMINAN
if (isset($_GET['hapus'])) {
    $id_hapus = intval($_GET['hapus']);
    mysqli_query($koneksi, "DELETE FROM jenis_jaminan WHERE id_jaminan = '$id_hapus'");
    header("Location: master_jaminan.php");
    exit();
}

$list_jaminan = mysqli_query($koneksi, "SELECT * FROM jenis_jaminan ORDER BY id_jaminan DESC");
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <title>Master Jenis Jaminan - Admin Panel</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="container-fluid p-4 flex-grow-1">
            <h4 class="mb-4 text-dark fw-normal">Manajemen Syarat Jenis Jaminan Fisik</h4>
            <?= $pesan; ?>

            <div class="row g-4">
                <div class="col-xl-4">
                    <div class="card border-0 shadow-sm p-4 bg-white rounded-3">
                        <h6 class="fw-bold mb-3 text-secondary text-uppercase">Tambah Jaminan</h6>
                        <form action="" method="POST">
                            <div class="mb-2">
                                <label class="small fw-bold mb-1">Nama Jaminan</label>
                                <input type="text" name="nama_jaminan" class="form-control form-control-sm" placeholder="Contoh: KTP Asli / Motor + STNK" required>
                            </div>
                            <div class="mb-3">
                                <label class="small fw-bold mb-1">Keterangan Aturan</label>
                                <textarea name="keterangan" class="form-control form-control-sm" rows="3" placeholder="Aturan penahanan jaminan fisik..." required></textarea>
                            </div>
                            <button type="submit" name="simpan_jaminan" class="btn btn-orange btn-sm w-100 fw-bold py-2">SIMPAN JAMINAN</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="col-xl-8">
                    <div class="card border-0 shadow-sm p-4 bg-white rounded-3">
                        <h6 class="fw-bold mb-3 text-secondary text-uppercase">Daftar Jenis Jaminan Berlaku</h6>
